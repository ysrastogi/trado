...
class DerivMessageHandler(IMessageHandler):
    ...
    def __init__(
        ...
        """
        self.logger = logger.getChild("DerivMessageHandler")
    ...
    def handle_message(self, data: Any) -> None:
        ...
        if not isinstance(data, dict):
            self.logger.error(f"Expected dict data, got {type(data)}")
            return
        ...
        if msg_type == 'authorize':
            ...
            # This is a response to our ping, no need to respond
            pass
    ...
    def _handle_ohlc_data(self, data: Dict[str, Any]) -> None:
        ...
        if data.get('ohlc'):
            ...
            if self.redis_publisher:
                try:
                    self.redis_publisher.publish_ohlc(ohlc_data)
                except Exception as e:
                    self.logger.error(f"Failed to publish OHLC to Redis Stream: {e}")
